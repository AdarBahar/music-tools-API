...
The `.codeagent/` directory is split intentionally:

* **`current/` = state of the system**
  Living, project-specific source of truth. What exists today, how it works, and how it is operated.

* **`prompts/` = how work is done**
  Reusable workflow instructions for planning, implementing, cleaning up, and documenting changes.

Think of it as:

* `current/` answers *"what is true right now"*
* `prompts/` answers *"what should I do next and how"*
...
Empty files are valid, but must be progressively filled.
Do not assume content exists. Create sections when missing.
